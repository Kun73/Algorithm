...
{0} & {\frac{1}{2}\left(1+\rho_{w}^{2}\right)} & {\alpha^{2} \frac{\left(1+\rho_{\nu}^{2}\right) \rho_{w}^{2}}{\left(1-\rho_{w}^{2}\right)}} \\
...
\begin{align}
a_{23}a_{32}&\leq \frac{1}{\Gamma}(1-a_{22})(1-a_{33}),\\
(\#eq:gamma1)
\end{align}
...
Next, we derive what exactly conditions $\alpha$ should satisfy to make the inequalities \@ref(eq:gamma1) and \@ref(eq:gamma2) hold. Additionally, in the proof of building linear inequality system of $E\left[\Vert Y_{k+1}-\mathbf{1}\bar y_{k+1}\Vert^2|\mathcal{F}_k\right]$, the author used 
\begin{align}
...
\end{align}
Thus we also need $\beta>0$.

For inequality \@ref(eq:gamma1), 

\begin{align}
a_{23}a_{32}&=\alpha^{2} \frac{\left(1+\rho_{w}^{2}\right) \rho_{w}^{2}}{\left(1-\rho_{w}^{2}\right)}\left[\left(\frac{1}{\beta}+2\right)\|W-I\|^{2} L^{2}+3 \alpha L^{3}\right]\\
&=
\end{align}
